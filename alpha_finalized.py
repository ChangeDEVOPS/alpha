...
max_priceEarningsRatio= float(input\
    ...
    https://www.investopedia.com/terms/p/price-earningsratio.asp (e.g 50) (min == 0,default): "))
max_priceToBookRatio = float(input\
    ("Enter maximum value for PB Ratio \n\
    https://www.investopedia.com/terms/p/price-to-bookratio.asp (e.g 50) (min == 0,default): "))
min_returnOnEquity = float(input\
    ("Enter minimum value for ROE\n\
    https://www.investopedia.com/terms/r/returnonequity.asp (e.g 0.05) (min == 0,default): "))
min_netProfitMargin = float(input\
    ("Enter minimum value for Profit Margin\n\
    https://www.investopedia.com/terms/p/profitmargin.asp (e.g 0.05) (min == 0,default): "))

for item in query_data:
    stock_ticker = item['symbol']
    stock_ticker = str(stock_ticker).replace(".","")
    query_ratios = requests.get \
        ((f'https://fmpcloud.io/api/v3/ratios/FB?apikey={API_key}'))
    query_ratios = query_ratios.json()
    current_year_ratios = query_ratios[0].items()
    # print()
